...
Object.values(placeholderData.staffDetails).forEach(staffList => {
    staffList.forEach(s => {
        if (!s.discipleship) {
            ...
            // Calculate highest level for the progress bar
            ...
            s.discipleshipLevel = maxIdx === 0 ? 1 : maxIdx; // map to 1-4 for UI
        }
    });
});
...
// Generates a simple colored dot for the map instead of flag images
function createDotIcon(isSensitive = false) {
    // PDF dictates larger dots for sensitive countries
    const d = isSensitive ? 24 : 14; 
    return L.divIcon({
        html: `<div style="background:#db5729;width:${d}px;height:${d}px;border-radius:50%;border:2px solid white;box-shadow:0 0 6px rgba(0,0,0,.4);"></div>`,
    ...
    if (staff.length) {
        staff.forEach(s => {
            ...
            html += `<div class="staff-card">
                ...
                ${discHtml}
                <p><span class="label" style="margin-top:5px; display:block;">Overall Level</span>
...
function buildChurchSidebar(ch) {
    ...
    let html = `<div class="sb-header"><h2>${ch.name}</h2></div>${notice}<hr>`;
...
function buildCountrySidebar(country) {
    ...
    let html = `<div class="sb-header"><h2>${country.name}</h2></div><hr>
        <div class="stat-grid">
...
function buildStateSidebar(stateName, stateData) {
    let html = `<div class="sb-header"><h2>${stateName}</h2></div><hr>`;
    if (stateData) {
...
function applySecurityMode(mode) {
    ...
    // Update button text to reflect state without emojis
    const statusText = { normal: 'Security: Off', obfuscate: 'Security: Obfuscated', hidden: 'Security: Hidden' };
    securityBtn.textContent = statusText[pinSecurityMode];
